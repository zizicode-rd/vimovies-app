...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!monitor) {
    return buildMetadata({
      ...
      description: 'Ficha técnica del monitor en Vimovies.',
      type: 'website',
      noIndex: true,
    });
  }
...
export default async function MonitorPage({ params }: PageProps) {
  ...
  const breadcrumb = jsonLdBreadcrumb([
    ...
    { name: name, url: `https://vimovies.com/es/monitores/${brand}/${slug}` },
  ]);
  ...
  const productLd = JSON.stringify({
    ...
    url: `https://vimovies.com/es/monitores/${brand}/${slug}`,
